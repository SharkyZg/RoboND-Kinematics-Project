...
The assignment of this project was to solve the inverse kinematics problem of the KUKA R210 robot arm. The problem has been solved by calculating angles for 6 joints of the robot arm with the help of inverse kinematics to achieve end-effector position given by the path planner. 
...
To find a wrist center position (position of the 5th joint) from a given gripper position, we first add previously calculated T0_G matrix to the correction matrix as defined bellow:
![alt text][image4]
```
...
After the differences between URDF and DH conventions have been addressed, we extract R_corr_rot rotation matrix:
```
R_corr_rot = R_corr[0:3, 0:3]
```

Then we define rotation matrices around x, y and z axes:
